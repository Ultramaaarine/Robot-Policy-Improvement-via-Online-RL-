#这是最初的dataset 本体为放在dataset文件夹里面的 Task_D_D 或者Task_ABC_D或者debug
#demo 是从这里提取出来的
# 这里面有 visual obs action depth scene obs 的数据 可用于对 online 数据归一化
import os
import re
import torch
import logging
...
class NormDataset(BaseDataset):
    def __init__(self, *args, **kwargs):
        super(NormDataset, self).__init__(*args, **kwargs)
    ...
    def lookup_naming_pattern(self):
        ...
        naming_pattern = [filename.parent / aux_naming_pattern[0], filename.suffix]#Path的/运算符是路径拼接
    ...
    def zip_sequence(self, start_idx: int, end_idx: int):
        """
    ...
    def load_file_indices(self, data_dir: Path, skill: str) -> Tuple[List, List]:
        ...
        all_eps_idx_part_task = [i for (i, v) in enumerate(data["language"]["task"]) if v == skill_name] #找出所有任务名称（v） skill（skill_name）的 demo index i。
        all_eps_start_end_part_task = [data["info"]["indx"][i] for i in all_eps_idx_part_task] # 告诉 第 i 条 demo 对应第几个帧 data["info"]["indx"][i] == (start_idx, end_idx)
