...
from sac_diffusion.utils.rollout1 import rollout
...
class TrainDiffusionWorkspace(BaseWorkspace):
    ...
    def _critic_train_step_from_window(self, critic_window):
        ...
        (n_cond, next_n_cond, visual_cond,next_visual_cond, stage_out, next_stage_out) = assemble_conditions(cond_t=cond_t,
        ...
        with torch.no_grad():
            q1_detach, q2_detach = self.critic_network(state=n_cond, visual_obs=visual_cond, action=critic_action)
            q_detach = torch.min(q1_detach, q2_detach)

        v_value = self.v_net(visual_obs=next_visual_cond, low_dim_obs=n_cond)
    ...
    def _training_diffusion(self):
        ...
        for epoch_idx in range(cfg.training.num_epochs):
            ...
            self.writer.add_scalar("training/epoch_loss", train_loss, global_step=self.global_step)
    ...
    def _policy_improvement(self, num_steps: int = 400):
        ...
        with tqdm.tqdm(total=num_steps, desc="Improve Policy", mininterval=1.0) as pbar:
            for improve_step in range(num_steps):
                ...
                (n_cond, next_n_cond, visual_cond,next_visual_cond, stage_out, next_stage_out) = assemble_conditions(
                    cond_t=cond_t,
                    cond_norm_params=self.cond_norm_param_dict,
                    batch_size=batch_size,
                    device=self.device,
                ...
                with torch.no_grad():
                    q1, q2 = self.critic_network(
                        state=n_cond,
                        visual_obs=visual_cond,
                        action=improve_action,
                    )
                    q = torch.min(q1, q2)

                    v = self.v_net(
                        visual_obs=next_visual_cond,
                ...
                self.writer.add_scalar("improve/loss", improve_log["loss"], self.global_step)
                self.writer.add_scalar("improve/weight_mean", improve_log["weight_mean"], self.global_step)
                self.writer.add_scalar("improve/weight_max", improve_log["weight_max"], self.global_step)
                self.writer.add_scalar("improve/adv_mean", improve_log["adv_mean"], self.global_step)
                self.writer.add_scalar("improve/adv_max", improve_log["adv_max"], self.global_step)
...
@hydra.main(
    ...
    config_name="ddpm_critic(Unet)_training",
    version_base="1.1"
)
def main(cfg):
