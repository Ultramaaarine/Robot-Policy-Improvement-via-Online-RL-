...
from sac_diffusion.policy.hybrid_transformer_ddim_policy import DDIMHybridPolicy
...
class TrainDiffusionWorkspace(BaseWorkspace):
    def __init__(
        ...
        # ---------- models ----------
        self.diffusion_model: DDIMHybridPolicy = hydra.utils.instantiate(cfg.policy)
    ...
    def _training_diffusion(self):
        ...
        for epoch_idx in range(cfg.training.num_epochs):
            ...
            if self.epoch >= rollout_after and self.epoch % rollout_every == 0:
               offline_rollout_result = self._rollout(select_with_critic=False) # check base policy
        # ============================================================
        ...
        for online_cycle in range(1, self.cfg.training.max_online_cycle + 1):
            ...
            if online_cycle  > 2:
                self._policy_improvement(num_steps=100)
            elif online_cycle <=2:
...
@hydra.main(
    ...
    config_name="ddim_critic_training",
    version_base="1.1"
)
def main(cfg):
